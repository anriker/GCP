...
int TabuSearch::C_TabuSearch(Graph &G,int Color_num) {//禁忌搜索
	...
	while (1) {
		...
		if (punishment == 0) {
			...
			Change_Color(G, Color_num);//初始可行解
			tabuList(G, Color_num);
			punishment = funcation(G, Color_num);
			cout << "\n" << punishment << " ";//重复搜索
			best_p = punishment;
...
void TabuSearch::aspiration_color_table(Graph &G, int Color_num)//定义邻接颜色表Adjacent_Color_Table[N][K]
{
	...
	cout << Adjacent_Color_Table[temp + k] << " ";
	}cout << endl;
	}*/
}
...
void TabuSearch::MakeMove(Graph &G, int Color_num, Find_u_vi_vj &U) {
	...
	else */TabuTenure[U.u*Color_num + U.vi] = punishment + rand() % 10 + iter;//修改禁忌表
...
int TabuSearch::funcation(Graph &G, int Color_num) {//目标函数，即记录惩罚(冲突对数)
	int punish = 0;
